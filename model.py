...
def compute_positional_div_term(d_model):
    ...
    return torch.exp((-math.log(10000)*2*i)/d_model)
    pass
...
def softmax_attention_weights(masked_scores):
    ...
    attn_score = torch.softmax(masked_scores,dim=-1)
...
def scaled_dot_product_attention(query, key, value, mask=None):
    ...
    scaled_raw_score = scale_attention_scores(raw_score, query.size(-1))

    if mask is not None:
        scaled_raw_score = mask_attention_scores_with_neg_inf(scaled_raw_score, mask)
    attn_score = softmax_attention_weights(scaled_raw_score)

    context = apply_attention_weights_to_values(attn_score, value)
...
def assemble_multi_head_attention_forward(query, key, value, w_q, w_k, w_v, w_o, num_heads, mask=None):
    ...
    context_h, weights_h = multi_head_scaled_dot_product_attention(q_h, k_h, v_h, mask=mask)

    return merge_heads_and_project_output(context_h,w_o,b_o=None)
...
def encoder_layer_self_attention_sublayer(x, w_q, w_k, w_v, w_o, gamma, beta, num_heads, src_mask):
    ...
    return apply_residual_add_and_norm(x,assemble_multi_head_attention_forward(x, x, x, w_q, w_k, w_v, w_o, num_heads, mask=src_mask), gamma, beta, eps=1e-5)
...
def encoder_layer_feed_forward_sublayer(x, w1, b1, w2, b2, gamma, beta):
    ...
    return apply_residual_add_and_norm(x,position_wise_feed_forward_network(x, w1, b1, w2, b2),gamma, beta, eps=1e-5)
...
def stack_encoder_layers(x, encoder_layer_params_list, num_heads, src_mask):
    ...
    for layer_params in encoder_layer_params_list:
        out = assemble_encoder_layer(out, layer_params, num_heads, src_mask)
...
def decoder_layer_masked_self_attention_sublayer(y, w_q, w_k, w_v, w_o, gamma, beta, num_heads, tgt_mask):
    ...
    out = assemble_multi_head_attention_forward(y,y,y,w_q, w_k, w_v, w_o, num_heads, mask=tgt_mask)
...
def decoder_layer_cross_attention_sublayer(y, encoder_output, w_q, w_k, w_v, w_o, gamma, beta, num_heads, src_mask):
    ...
    out = assemble_multi_head_attention_forward(y, encoder_output, encoder_output, w_q, w_k, w_v, w_o, num_heads, mask=src_mask)
...
def decoder_layer_feed_forward_sublayer(y, w1, b1, w2, b2, gamma, beta):
    ...
    return apply_residual_add_and_norm(y, ffn_out, gamma, beta, eps=1e-5)
